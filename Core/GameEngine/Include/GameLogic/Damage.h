...
enum DamageType CPP_11(: Int)
{
	...
	DAMAGE_SUBDUAL_FROZEN,  ///< Accumulates frozen damage; disables target when threshold is reached
	DAMAGE_SUBDUAL_FROZEN_UNRESISTABLE,  ///< Used for recovery from SUBDUAL_FROZEN
...
inline Bool IsSubdualFrozenDamage( DamageType type )
{
	switch( type )
	{
		case DAMAGE_SUBDUAL_FROZEN:
		case DAMAGE_SUBDUAL_FROZEN_UNRESISTABLE:
			return TRUE;
	}
